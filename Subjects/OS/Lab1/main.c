...
void daemonize(const char *cmd)
{
    ...
    umask(0);
    printf("1.Сброшена маска режима создания файла\n");
    // 2. Получение максимального возможного номера дискриптора
    if (rl.rlim_max = getrlimit(RLIMIT_NOFILE, &rl) < 0)
        perror("Невозможно получить максимальный номер дискриптора!\n");
    printf("2.Получили максимальный возможный номер дискриптора.\n");
    ...
    if ((pid = fork()) < 0)
        ...
        exit(0);
    setsid();
    printf("3.Поставили процесс лидером новой сессии.\n");
    ...
    if ((pid = fork()) < 0)
        ...
        exit(0);
    printf("4.Обеспечили невозможность обретения терминала сессии.\n");
